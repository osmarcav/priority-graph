...
export class RoadmapGraph {
  ...
  private getAllDescendants(nodeId: string): string[] {
    ...
    while (queue.length > 0) {
      ...
      const childIds = this.children.get(current) ?? [];
  ...
  getDownstreamEffort(nodeId: string): number {
    ...
    while (queue.length > 0) {
      ...
      // Find nodes that DEPEND_ON current (current blocks them)
      const _blockedEdges =
        this.outgoing
          .get(current)
          ?.filter(
  ...
  > {
    ...
    for (const edge of derivedEdges) {
      ...
      agg.byType.set(edge.type, (agg.byType.get(edge.type) ?? 0) + edge.weight);
    }
